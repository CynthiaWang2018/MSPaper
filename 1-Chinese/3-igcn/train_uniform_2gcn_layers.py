...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset', 'law', 'Dataset string.')  # 'cora', 'citeseer', 'pubmed'
...
#             print('now distr is ', distr)
            if rank1 > len(distr):
                q1 = distr
            else:
                q1 = np.random.choice(distr, rank1, replace=False)

            support1 = sparse_to_tuple(normADJ_batch[:, q1] * numNode_train_1 / len(q1))

            if rank0 > len(distr):
                q0 = distr
            else:
                q0 = np.random.choice(distr, rank0, replace=False)

            support0 = sparse_to_tuple(normADJ_train_vocab[q1, :][:, q0])

            # p1 = column_prop(normADJ_batch)
            # # print(p1.shape)
            # q1 = np.random.choice(np.arange(numNode_train_1), rank1, replace=False)  # top layer
            ...
            # # print(p2.shape)
            # q0 = np.random.choice(np.arange(numNode_train_1), rank0, replace=False)
            # support0 = sparse_to_tuple(normADJ_train_vocab[q1, :][:, q0])
...
if __name__ == "__main__":
    ...
    for k in [50, 100, 200, 400, 600, 800]:
        main(k, k)
