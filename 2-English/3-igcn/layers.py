...
class GraphConvolution(Layer):
    ...
    def __init__(self, input_dim, output_dim, placeholders, dropout=0.,
        ...
        self.act = act
        print('the activation function', self.act)
        if support is None:
    ...
    def _call(self, inputs):
        ...
        if not self.featureless:
            pre_sup = dot(x, self.vars['weights_0'],
                          sparse=self.sparse_inputs)
            print('featureless is false')
        else:
            pre_sup = self.vars['weights_0']
            print('featuresless is true')
...
class GraphPageRank(Layer):
    ...
    def __init__(self, input_dim, output_dim, alpha, propagations, placeholders,support=None, dropout=0.,
        ...
        with tf.variable_scope(self.name + '_vars'):
            ...
            self.vars['weights1'] = glorot([input_dim, output_dim], name='weights1')
    ...
    def _call(self, inputs):
        ...
        # for i in range(len(self.support)):
            ...
            #             else:
            #                 pre_sup = self.vars['weights_' + str(i)]
        out1 = dot(x, self.vars['weights1'],sparse=self.sparse_inputs)
        # x xw
        # [nan,200][200,20]->nan,20
        ...
        tempA = self.support  # A0
        print('tempA.shape1',tempA.shape)
        support = out1 # Z0
        print('support.shape1',support.shape) #[256,600]
        for k in range(self.propagations):
            support = (1 - self.alpha) * dot(tempA, support, sparse=True) + self.alpha * out1  # modify
            # [256,600] [600,8] + [600,8] ->[256,8]
            #[256,600][256,8]+[]
            print('support.shape2',support.shape)
